...
if (process.argv.length < 4) {
    console.error('Usage: jupyter-devinstall <githubUserName> <installDir> [--overwrite] [--reinstall] [--global]');
    process.exit(1);
}
...
let downloaded;
let defaultInstallScope = flags.global ? 'g' : 'l';
if (flags.reinstall) {
    downloaded = Promise.resolve({install: defaultInstallScope});
} else {
    downloaded = Promise.resolve().then(() => {
...
downloaded.then(config => {
    if (config.install === 'l' || config.install === 'g') {
        ...
        for (let i = 0; i < orgRepos.length; i++) {
            ...
            let shell = 'python -m pip install ' + localFlag + '-e ' + path.resolve(expandTilde(installdir), orgRepo);
